...
export default function ChannelScreen({ onJoinChannel, userProfile }) {
  ...
  const channelGroups = [
    { title: 'KANTOR PUSAT', icon: Building2, accent: '#2563eb',
      items: ['Departemen HC, Pengembangan dan IT','Departemen Keuangan, Akuntansi, dan Risiko','Departemen Operasi I','Departemen Operasi II','Departemen QHSSE','Departemen Satuan Pengawas Intern','Departemen Sekretaris Perusahaan','Unit ITRS','Unit OCSC','Unit SMRK','Unit Bisnis Turunan OM'] },
    { title: 'RUAS', icon: MapPin, accent: '#059669',
      items: ['Ruas BTB','Ruas Akses Tanjung Priok','Ruas Bakauheni–Terbanggi Besar','Ruas Bengkulu–Taba Penanjung','Ruas Betung–Jambi','Ruas Binjai–Stabat','Ruas Indralaya–Prabumulih','Ruas Indrapura–Kisaran','Ruas JORRS','Ruas Kuala Tanjung–Parapat','Ruas Medan–Binjai','Ruas Padang–Sicincin','Ruas Palembang–Indralaya','Ruas Pekanbaru–Bangkinang','Ruas Pekanbaru–Dumai','Ruas Sigli–Banda Aceh','Ruas Terbanggi Besar–Kayu Agung'] },
    { title: 'UNIT PRODUKSI', icon: Factory, accent: '#d97706',
      items: ['UP Bojonegara','UP Indralaya','UP Jabodetabek','UP Muara Fajar','UP Patimban','UP Sei Langsat','UP Stone Crusher Sumatera'] }
  ];
  ...
  const filteredGroups = search.trim()
    ? [{ title: 'HASIL PENCARIAN', icon: Search, accent: '#7c3aed', items: allItems.filter(i => i.toLowerCase().includes(search.toLowerCase())) }]
    : channelGroups;
  ...
  return (
    <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', background: 'var(--bg-primary)' }}>
      ...
      {/* Header */}
      <div style={{ background: 'var(--bg-secondary)', borderBottom: '1px solid var(--border)', padding: '0.75rem 1rem', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: '0.95rem', fontWeight: 700, letterSpacing: '-0.02em' }}>Saluran PTT</div>
            <div style={{ fontSize: '0.55rem', color: 'var(--text-tertiary)', fontFamily: "'JetBrains Mono', monospace", letterSpacing: '0.04em', display: 'flex', alignItems: 'center', gap: '4px' }}>
              <Wifi size={8} /> SIARAN LANGSUNG
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Search */}
      <div style={{ padding: '0.5rem 0.8rem', borderBottom: '1px solid var(--border)', flexShrink: 0, background: 'var(--bg-secondary)' }}>
        <div style={{ position: 'relative', marginBottom: '0.35rem' }}>
          <Search size={13} style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', color: 'var(--text-tertiary)' }} />
          <input placeholder="Cari saluran..." value={search} onChange={(e) => setSearch(e.target.value)} style={{ ...inputBase, width: '100%', paddingLeft: '30px' }} />
        </div>
        <div style={{ display: 'flex', gap: '0.3rem' }}>
          <button onClick={() => setShowCreate(!showCreate)} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '3px', padding: '7px', border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', background: 'var(--bg-tertiary)', color: 'var(--text-secondary)', fontSize: '0.65rem', fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>
            <Plus size={11} /> Buat Baru
          </button>
          <form onSubmit={handleJoinChannel} style={{ flex: 1, display: 'flex', gap: '3px' }}>
            <input placeholder="Kode..." value={joinCode} onChange={(e) => setJoinCode(e.target.value)} style={{ ...inputBase, flex: 1, textTransform: 'uppercase', fontFamily: "'JetBrains Mono', monospace", fontSize: '0.7rem' }} />
            <button type="submit" disabled={!joinCode.trim()} style={{ background: joinCode.trim() ? 'var(--accent-blue)' : 'var(--bg-tertiary)', color: 'white', border: '1px solid transparent', borderRadius: 'var(--radius-sm)', padding: '0 8px', display: 'flex', alignItems: 'center', cursor: 'pointer' }}><LogIn size={12} /></button>
          </form>
        </div>
        {showCreate && (
          <form onSubmit={handleCreateChannel} style={{ display: 'flex', gap: '3px', marginTop: '0.35rem', animation: 'slideIn 0.2s var(--ease-out)' }}>
            <input autoFocus placeholder="Nama saluran baru..." value={newChannel} onChange={(e) => setNewChannel(e.target.value)} style={{ ...inputBase, flex: 1, borderColor: 'var(--accent-border)' }} />
            <button type="submit" disabled={!newChannel.trim()} style={{ background: newChannel.trim() ? 'var(--accent)' : 'var(--bg-tertiary)', color: 'white', border: 'none', borderRadius: 'var(--radius-sm)', padding: '0 12px', fontWeight: 600, fontSize: '0.65rem', cursor: 'pointer', fontFamily: 'inherit' }}>BUAT</button>
          </form>
        )}
      </div>
        ...
        {favChannel && !search.trim() && (
          <div style={{ padding: '0.6rem 0.8rem 0' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '5px', marginBottom: '0.3rem' }}>
              ...
              <span style={{ fontSize: '0.55rem', fontWeight: 700, color: '#d97706', letterSpacing: '0.1em', fontFamily: "'JetBrains Mono', monospace" }}>SALURAN SAYA</span>
            </div>
            <div 
              onClick={() => onJoinChannel(getChannelCode(favChannel))} 
              style={{ background: 'rgba(217,119,6,0.04)', border: '1px solid rgba(217,119,6,0.12)', borderRadius: 'var(--radius-md)', padding: '0.7rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.6rem', transition: 'all 0.2s var(--ease-out)' }}
            >
              <div style={{ width: 40, height: 40, borderRadius: 'var(--radius-sm)', background: '#d97706', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                <Star size={16} color="white" />
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontWeight: 600, fontSize: '0.82rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{favChannel}</div>
                <div style={{ fontSize: '0.55rem', color: '#d97706', fontWeight: 600, fontFamily: "'JetBrains Mono', monospace", letterSpacing: '0.02em', marginTop: '1px', display: 'flex', alignItems: 'center', gap: '3px' }}>
